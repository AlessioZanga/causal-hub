...
rs_attrs += "use std::{\n"
rs_attrs += "    collections::{hash_set, HashSet},\n"
rs_attrs += "    hash::{Hash, Hasher},\n"
rs_attrs += "};\n"
rs_attrs += "\n"
...
for (w, (k, v)) in enumerate(used_by.items()):
    ...
    rs_used_by += "    attributes: HashSet<Attribute>,\n"
    rs_used_by += "}\n"
    rs_used_by += "\n"
    ...
    for i in v:
        rs_used_by += f"            \"{i}\" => Attribute::{sc2cm(i)}(value.into()),\n"
    rs_used_by += "            _ => panic!(\"Invalid attribute key `{key}` for " + f"{k.removesuffix('s')}Attributes\"),\n"
    rs_used_by += "        };\n"
    rs_used_by += "\n"
    ...
    for (j, i) in enumerate(v):
        ...
        rs_used_by += f"        let item = Attribute::{sc2cm(i)}(s.into());\n"
        rs_used_by += "\n"
        rs_used_by += "        self.attributes.replace(item).is_none()\n"
        rs_used_by += "    }\n"
        rs_used_by += "\n"
    ...
    rs_used_by += "\n"
    rs_used_by += "    type IntoIter = hash_set::IntoIter<Attribute>;\n"
    rs_used_by += "\n"
    rs_used_by += "    #[inline]\n"
