#[cfg(test)]
mod tests {
    use causal_hub::{map, utils::CMI};
    use ndarray::prelude::*;

    #[test]
    fn test_constrained_multi_index_ravel_empty() {
        // Set shape.
        let shape = array![2, 3, 4];
        // Create a multi index.
        let multi_index = CMI::new(shape.clone(), Default::default());

        // Assert the cardinality.
        assert_eq!(multi_index.cardinality(), &shape);

        // Assert the ravel multi index explicitly.
        assert_eq!(multi_index.ravel([0, 0, 0]), 0);
        assert_eq!(multi_index.ravel([0, 0, 1]), 1);
        assert_eq!(multi_index.ravel([0, 0, 2]), 2);
        assert_eq!(multi_index.ravel([0, 0, 3]), 3);
        assert_eq!(multi_index.ravel([0, 1, 0]), 4);
        assert_eq!(multi_index.ravel([0, 1, 1]), 5);
        assert_eq!(multi_index.ravel([1, 0, 0]), 12);

        // Assert the ravel multi index implicitly.
        for i in 0..shape[0] {
            for j in 0..shape[1] {
                for k in 0..shape[2] {
                    let ravel_index = multi_index.ravel([i, j, k]);
                    let expected_index = i * shape[1] * shape[2] + j * shape[2] + k;
                    assert_eq!(ravel_index, expected_index);
                }
            }
        }
    }

    #[test]
    fn test_constrained_multi_index_ravel() {
        // Set shape.
        let shape = array![2, 3, 4];
        // Create a multi index, forbid index 1 on axis 2.
        let multi_index = CMI::new(shape.clone(), vec![map![(2, 1)]]);

        // Assert the cardinality.
        assert_eq!(multi_index.cardinality(), &shape);

        // Assert the ravel multi index explicitly.
        assert_eq!(multi_index.ravel([0, 0, 0]), 0);
        // Will panic: assert_eq!(multi_index.ravel([0, 0, 1]), 1);
        assert_eq!(multi_index.ravel([0, 0, 2]), 1);
        assert_eq!(multi_index.ravel([0, 0, 3]), 2);
        assert_eq!(multi_index.ravel([0, 1, 0]), 3);
        // Will panic: assert_eq!(multi_index.ravel([0, 1, 1]), 5);
        assert_eq!(multi_index.ravel([1, 0, 0]), 9);
    }

    #[test]
    #[should_panic(expected = "Index `[0, 0, 1]` not allowed.")]
    fn test_constrained_multi_index_ravel_panic() {
        // Set shape.
        let shape = array![2, 3, 4];
        // Create a multi index, forbid index 1 on axis 2.
        let multi_index = CMI::new(shape.clone(), vec![map![(2, 1)]]);

        // Assert the cardinality.
        assert_eq!(multi_index.cardinality(), &shape);

        // Assert the ravel multi index explicitly.
        assert_eq!(multi_index.ravel([0, 0, 0]), 0);
        assert_eq!(multi_index.ravel([0, 0, 1]), 1);
    }

    #[test]
    fn test_constrained_multi_index_unravel_empty() {
        // Set shape.
        let shape = array![2, 3, 4];
        // Create a multi index.
        let multi_index = CMI::new(shape.clone(), Default::default());

        // Assert the unravel multi index explicitly.
        assert_eq!(multi_index.unravel(0), vec![0, 0, 0]);
        assert_eq!(multi_index.unravel(1), vec![0, 0, 1]);
        assert_eq!(multi_index.unravel(2), vec![0, 0, 2]);
        assert_eq!(multi_index.unravel(3), vec![0, 0, 3]);
        assert_eq!(multi_index.unravel(4), vec![0, 1, 0]);
        assert_eq!(multi_index.unravel(5), vec![0, 1, 1]);
        assert_eq!(multi_index.unravel(12), vec![1, 0, 0]);

        // Assert the unravel multi index implicitly.
        for i in 0..shape[0] {
            for j in 0..shape[1] {
                for k in 0..shape[2] {
                    let ravel_index = i * shape[1] * shape[2] + j * shape[2] + k;
                    let multi_index_value = multi_index.unravel(ravel_index);
                    assert_eq!(multi_index_value, vec![i, j, k]);
                }
            }
        }
    }

    #[test]
    fn test_constrained_multi_index_unravel() {
        // Set shape.
        let shape = array![2, 3, 4];
        // Create a multi index, forbid index 1 on axis 2.
        let multi_index = CMI::new(shape.clone(), vec![map![(2, 1)]]);

        // Assert the cardinality.
        assert_eq!(multi_index.cardinality(), &shape);

        // Assert the unravel multi index explicitly.
        assert_eq!(multi_index.unravel(0), vec![0, 0, 0]);
        // Will panic: assert_eq!(multi_index.unravel(1), vec![0, 0, 1]);
        assert_eq!(multi_index.unravel(1), vec![0, 0, 2]);
        assert_eq!(multi_index.unravel(2), vec![0, 0, 3]);
        assert_eq!(multi_index.unravel(3), vec![0, 1, 0]);
        // Will panic: assert_eq!(multi_index.unravel(5), vec![0, 1, 1]);
        assert_eq!(multi_index.unravel(9), vec![1, 0, 0]);
    }
}
