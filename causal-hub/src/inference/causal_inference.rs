use crate::{
    inference::{BNInference, BackdoorCriterion, ParBNInference},
...
where
    E: BNInference<CatBN>,
{
    fn cace_estimate(&self, x: &Set<usize>, y: &Set<usize>, z: &Set<usize>) -> Option<CatCPD> {
...
impl<E> ParBNCausalInference<CatBN> for CausalInference<'_, E>
where
    E: ParBNInference<CatBN>,
{
    fn par_cace_estimate(&self, x: &Set<usize>, y: &Set<usize>, z: &Set<usize>) -> Option<CatCPD> {
        // Assert X is not empty.
        assert!(!x.is_empty(), "Variables X must not be empty.");
        // Assert Y is not empty.
        assert!(!y.is_empty(), "Variables Y must not be empty.");
