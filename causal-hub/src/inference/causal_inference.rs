    ...
    T: BN,
{
    ...
    {
        self.csace_estimate(x, y, &set![], data)
    }
...
macro_for!($type in [CatBN, GaussBN] {
        ...
        E: Modelled<$type> + BNInference<$type>,
    {
        fn cpace_estimate(
            ...
            let w_ = &w.map_or(set![], |w| w.evidences().iter().filter_map(|evidence| evidence.clone().map(|evidence| evidence.event())).collect());
            ...
            let z_w_s = model.graph().find_minimal_backdoor_set(x, y, Some(z_w), None)?;
            ...
            match z_w_s {
                ...
                Some(z_w_s) => {
                    ...
                    let stats = p_y_s_do_x_z.indices_from(stats, model.labels())?;
                    // Marginalize over S.
                    let p_y_do_x_z = p_y_s_do_x_z.marginalize(&stats)?;
                    // Map BN indices to the potential indices.
                    let x = p_y_do_x_z.indices_from(x, model.labels())?;
                    let y = p_y_do_x_z.indices_from(y, model.labels())?;
                    let z = p_y_do_x_z.indices_from(z, model.labels())?;
                    // Convert back to CPD.
            ...
            D: Dataset<Evidence = <$type as BN>::Evidence>,
        {
            ...
            if self.engine.model().labels() != data.labels() {
            ...
            let u = Set::from_iter(0..data.labels().len());
            let u = &(&(&u - x) - y) - z;
            // Restrict the data to the variables in U.
            let d_prime = data.select(&u)?;
    ...
    T: BN,
{
    ...
    where
        D: Dataset<Evidence = T::Evidence>,
    {
        self.par_csace_estimate(x, y, &set![], data)
    }
        ...
        fn par_cpace_estimate(
            ...
            match z_w_s {
                ...
                Some(z_w_s) => {
                    ...
                    let p_s = self.engine.par_estimate(stats, &set![], None)?;
                    // Convert to potentials for aligned multiplication.
                    let p_y_x_z_s = p_y_x_z_s.into_phi()?;
                    let p_s = p_s.into_phi()?;
                    // Compute P(Y | X, Z, W = w, S) * P(S) using potentials.
    ...
    E: Modelled<MixedBN> + BNInference<MixedBN>,
{
    fn cpace_estimate(
        ...
        let z_w_s = model
            .graph()
            .find_minimal_backdoor_set(x, y, Some(z_w), None)?;
