    ...
    fn driver(c: &mut Criterion, model: &str) {
        ...
        for sample_size in [10, 100, 1_000, 10_000, 100_000].iter() {
            ...
            group.bench_with_input(
                ...
                |b, _| {
                    b.iter(|| {
                        let _ = LL::new(&data_set).marginal(black_box(0));
                    })
                },
            );
        }
...
pub mod par_marginal {
    ...
    fn driver(c: &mut Criterion, model: &str) {
        // Initialize benchmark group.
        let mut group =
            c.benchmark_group(format!("stats::log_likelihood::par_marginal::{model}").as_str());
        ...
        for sample_size in [10, 100, 1_000, 10_000, 100_000].iter() {
            ...
            group.bench_with_input(
                ...
                |b, _| {
                    b.iter(|| {
                        let _ = ParallelLL::new(&data_set).marginal(black_box(0));
                    })
                },
            );
        }
