pub mod call {
    ...
    fn driver(c: &mut Criterion, model: &str) {
        ...
        let mut group = c.benchmark_group(
            format!("stats::bayesian_information_criterion::call::{model}").as_str(),
        );
        ...
        for sample_size in [10, 100, 1_000, 10_000, 100_000].iter() {
            ...
            group.bench_with_input(
                ...
                |b, _| {
                    b.iter(|| {
                        let _ =
                            ScoringCriterion::call(&BIC::new(&data_set), black_box(model.graph()));
                    })
                },
            );
        }
...
pub mod par_call {
    ...
    fn driver(c: &mut Criterion, model: &str) {
        // Initialize benchmark group.
        let mut group = c.benchmark_group(
            format!("stats::bayesian_information_criterion::par_call::{model}").as_str(),
        );
        ...
        for sample_size in [10, 100, 1_000, 10_000, 100_000].iter() {
            ...
            group.bench_with_input(
                ...
                |b, _| {
                    b.iter(|| {
                        let _ = ScoringCriterion::call(
                            &ParallelBIC::new(&data_set),
