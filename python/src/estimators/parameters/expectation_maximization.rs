...
pub fn em<'a>(
    ...
    let output = py
        .detach(|| -> BackendResult<_> {
            ...
            let e_step = |prev_model: &CatCTBN,
                          evidence: &CatTrjsEv|
             -> BackendResult<CatWtdTrjs> {
            ...
            let em = EMBuilder::new(&model, evidence)
                ...
                .build()
                .map_err(|e| {
                    BackendError::Model(format!("Failed to build the EM algorithm: {}", e))
                })?;

            // Fit the model.
            em.fit().map_err(|e| {
                BackendError::Model(format!("Failed to fit the model using EM: {}", e))
            })
        })
